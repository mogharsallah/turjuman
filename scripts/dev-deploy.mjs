...
export async function devDeploy() {
	...
	const { Repository, TurjumanService, bootstrapOwner } = await import(
		"@turjuman/core"
	);
	...
	console.log(
		`Deploying "${STACK}" into LocalStack (arch ${ARCH}, hot-reload)...`,
	);
	...
	const email =
		process.env.TURJUMAN_DEV_OWNER_EMAIL ?? "dev-owner@turjuman.test";
	...
	if (existing) {
		...
		({ secret } = await new TurjumanService(repo).apiKeys.create(actor, {
			name: "dev",
		}));
	} else {
		({ user, secret } = await bootstrapOwner(repo, {
			email,
			name: "Dev Owner",
			force: true,
		}));
