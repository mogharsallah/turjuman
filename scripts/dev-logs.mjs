...
const COLORS = { mcp: "\x1b[34m", api: "\x1b[32m", webhook: "\x1b[35m", lambda: "\x1b[90m" };
const RESET = "\x1b[0m";
...
function unwrap(message) {
  const m = message.match(/^\S+\t\S+\t(?:INFO|WARN|ERROR|DEBUG|TRACE|FATAL)\t([\s\S]*)$/);
 ...
 * of the process.
 */
export async function streamLogs({ stackName, endpoint, region, pollMs = 1000 } = {}) {
      ...
      groups.map(async (group) => {
        ...
        try {
          const res = await client.send(
            new FilterLogEventsCommand({ logGroupName: group, startTime: since.get(group) ?? start }),
          );
          for (const ev of res.events ?? []) {
            // +1ms so the next poll's startTime doesn't re-emit this same event.
            since.set(group, Math.max(since.get(group) ?? 0, (ev.timestamp ?? 0) + 1));
            const line = unwrap(ev.message ?? "");
            if (line && !RUNTIME_NOISE.test(line)) console.log(`${color}[${label}]${RESET} ${line}`);
          }
        } catch {
  ...
  console.log("Streaming Lambda logs ([mcp]/[api]/[webhook]) — set DEV_LOGS=0 to disable.\n");
  await tick().catch(() => {});
