...
"@turjuman/core": minor
"@turjuman/api": minor
"@turjuman/mcp-server": patch
---

Unify structured logging across all three Lambdas. The MCP server's one-JSON-line-per-request logger (`logInfo`/`logError`/`errorInfo`) moves into `@turjuman/core` and is now shared:
