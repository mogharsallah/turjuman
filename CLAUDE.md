...
| `packages/mcp-server` | Stateless MCP over Streamable HTTP (one POST = one JSON-RPC message). Lambda handler in `src/handler.ts`; tool defs in `src/tools.ts`. `pnpm run build` also esbuild-bundles a self-contained `lambda/handler.mjs` asset (via `scripts/build-lambda.mjs`) that `@turjuman/aws-cdk` ships with `Code.fromAsset`. |
