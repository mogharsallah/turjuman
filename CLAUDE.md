...
| `packages/mcp-server` | Stateless MCP over Streamable HTTP (one POST = one JSON-RPC message). Lambda handler in `src/handler.ts`; tool defs in `src/tools.ts`. |
| `packages/api` | REST API for the CLI/CI (`src/router.ts`) + the Streams→webhook dispatcher (`src/webhook.ts`). |
...
| `packages/deploy` | The self-host tooling (`turjuman-deploy` bin, `src/main.ts`): a self-contained AWS deployer. The stack is declared with AWS CDK (`src/cdk.ts`) and deployed via the CDK programmatic toolkit (`src/toolkit.ts`) — no SAM CLI, no `cdk bootstrap`. Holds all the heavy AWS/CDK/esbuild deps, kept out of the sync CLI install. Reuses `@turjuman/cli/auth` to write/remove credentials. |
| `packages/e2e` | Black-box vitest specs that only talk HTTP to a deployed (LocalStack) stack. |
| `packages/deploy/src/cdk.ts` | AWS CDK stack: DynamoDB (on-demand, 3 GSIs, Streams, RETAIN) + three Lambda Function URLs. The single source of the deploy topology. |
