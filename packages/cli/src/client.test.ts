...
describe("ApiClient.request", () => {
  it("builds the URL (handling a missing trailing slash), sets the Bearer header, and parses JSON", async () => {
    ...
    const api = new ApiClient("https://api.example.com", "op_live_x", fetchImpl as unknown as typeof fetch);
    await api.listProjects();
    const [url, init] = fetchImpl.mock.calls[0]!;
    expect(String(url)).toBe("https://api.example.com/v1/projects");
    expect((init as RequestInit).headers).toMatchObject({ authorization: "Bearer op_live_x" });
  ...
  it("adds query params", async () => {
    ...
    const api = new ApiClient("https://api.example.com/", "k", fetchImpl as unknown as typeof fetch);
    await api.exportBundle("proj_1", "es-MX", { working: true, excludeStale: true });
  ...
  it("throws an API error (exit 3) with the server's error message on non-OK", async () => {
    const fetchImpl = vi.fn(async () => jsonResponse({ error: "nope" }, { status: 403 }));
    const api = new ApiClient("https://api.example.com", "k", fetchImpl as unknown as typeof fetch);
    await expect(api.listProjects()).rejects.toMatchObject({ message: "nope", exitCode: 3 });
  });
  ...
  it("surfaces the error code and requestId from the body", async () => {
    const fetchImpl = vi.fn(async () =>
      jsonResponse({ error: "Internal error", code: "INTERNAL", requestId: "req-123" }, { status: 500 }),
    );
    const api = new ApiClient("https://api.example.com", "k", fetchImpl as unknown as typeof fetch);
...
describe("ApiClient pagination", () => {
  ...
  it("listKeys follows pagination too", async () => {
    ...
    expect(new URL(String(fetchImpl.mock.calls[0]![0])).searchParams.get("namespace")).toBe("default");
  });
});

describe("ApiClient request-body validation", () => {
  it("rejects an invalid locale before sending (usage error, exit 2)", async () => {
    const fetchImpl = vi.fn(async () => jsonResponse({ written: 0, skipped: [] }));
    const api = new ApiClient("https://api.example.com", "k", fetchImpl as unknown as typeof fetch);
    await expect(
      api.importTranslations("proj_1", "NOT A LOCALE", [{ name: "k", value: "v" }]),
  ...
  it("sends a validated keys-import body", async () => {
    const fetchImpl = vi.fn(async () =>
      jsonResponse({ created: 1, updated: 0, reactivated: 0, baseValuesSet: 1, deleted: 0, deprecated: 0 }),
    );
    const api = new ApiClient("https://api.example.com", "k", fetchImpl as unknown as typeof fetch);
    await api.importKeys("proj_1", [{ name: "greeting", baseValue: "Hi" }], "default", { deprecate: true });
