...
export async function runPush(
			...
			if (locale.code === project.baseLocale) {
				if (opts.dryRun) {
					...
					files.push({
						...
						planned: {
							...
							existing: local.size - created.length,
				...
				out.line(
					`${file}: ${res.written} ${locale.code} translations written${
						res.skipped.length
							? `, ${res.skipped.length} skipped (unknown keys)`
							: ""
					}`,
				);
				files.push({
	...
	if (opts.check) {
		if (opts.dryRun)
			out.note("[dry-run] QA checks run against current remote state.");
...
export function registerPush(program: Command, deps: CliDeps): void {
	program
		.command("push")
		.description(
			"Upload local source/translation files. Base-locale file creates/updates keys.",
		)
		.option(
			"--prune",
			"Delete remote keys (per namespace) absent from the base-locale file",
		)
		.option("--dry-run", "Show what would change without writing anything")
		.option(
			"--check",
			"After pushing, run QA checks and exit non-zero on any error finding",
		)
		.action(
			async (opts: { prune?: boolean; dryRun?: boolean; check?: boolean }) => {
