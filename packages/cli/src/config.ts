...
export { AUTH_FILE, type AuthConfig, loadAuth, saveAuth, removeAuth } from "./auth.js";
...
function normalize(stored: StoredConfig): ProjectConfig {
  ...
  if (stored.format && stored.path) {
    return {
      ...
      targets: [{ format: stored.format, path: stored.path, namespace: stored.namespace }],
    };
  }
  throw usageError(`${CONFIG_FILE} must define at least one target (format + path).`);
}
