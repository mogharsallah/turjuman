...
export class ApiClient {
  ...
  private async request<T>(
    ...
    try {
      ...
      throw apiError(`Cannot reach ${url.origin}: ${err instanceof Error ? err.message : String(err)}`);
    }
    const text = await res.text();
    const data = text ? JSON.parse(text) : {};
    if (!res.ok) {
      const body = data as { error?: string; code?: string; requestId?: string };
      ...
      throw apiError(requestId ? `${message} (request ${requestId})` : message, {
        code: body.code,
        requestId,
      });
    }
  ...
  private async requestAllPages<E>(
    ...
    do {
      const page = await this.request<Record<string, unknown>>("GET", path, {
        query: { ...query, limit: String(PAGE_SIZE), ...(cursor ? { cursor } : {}) },
      });
  ...
  async listKeys(projectId: string, namespace?: string): Promise<{ keys: TranslationKey[] }> {
  ...
  async exportLocale(projectId: string, locale: string): Promise<{ translations: Translation[] }> {
  ...
  async importKeys(
    ...
    return this.request("POST", `v1/projects/${projectId}/keys/import`, { body });
  }
  ...
  async importTranslations(
    ...
    const body = validateBody(importTranslationsBodySchema, { locale, entries });
    return this.request("POST", `v1/projects/${projectId}/translations/import`, { body });
  }

  listFormats(): Promise<{ formats: { id: string; label: string; extensions: string[] }[] }> {
  ...
  async runChecks(
    projectId: string,
    opts: { locale?: string; checks?: string[]; slot?: "working" | "approved" } = {},
  ): Promise<qa.QaReport> {
