...
import { CONFIG_FILE, loadAuth, loadConfig, removeAuth, saveAuth, saveConfig } from "./config.js";
...
describe("loadConfig", () => {
  it("returns the multi-target form unchanged", () => {
    const dir = tmpRepo({
      projectId: "proj_1",
      targets: [
        { format: "json-nested", path: "a/{locale}.json", namespace: "default" },
  ...
  it("normalizes the legacy single-target form", () => {
    const dir = tmpRepo({ projectId: "proj_2", format: "json-flat", path: "i18n/{locale}.json" });
    expect(loadConfig(dir)).toEqual({
      projectId: "proj_2",
      targets: [{ format: "json-flat", path: "i18n/{locale}.json", namespace: undefined }],
    });
  });
...
describe("auth credentials", () => {
  const saved = { url: process.env.TURJUMAN_API_URL, key: process.env.TURJUMAN_API_KEY };
  afterEach(() => {
  ...
  it("uses env vars when both are set, without touching the auth file", () => {
    ...
    expect(loadAuth(tmpAuthDir())).toEqual({ url: "https://env.example.com", key: "op_live_env" });
  });
  ...
  it("merges a partial env override over the file", () => {
    ...
    expect(loadAuth(dir)).toEqual({ url: "https://env-only.example.com", key: "op_live_file" });
  });
