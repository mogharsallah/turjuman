...
describe("MCP protocol handler", () => {
	...
	it("lists tools with JSON Schemas", async () => {
		...
		expect(res.result.tools.every((t) => t.inputSchema.type === "object")).toBe(
			true,
		);
	});
	...
	it("renders a prompt via the scoring service on prompts/get", async () => {
		...
		const res = (await handleMessage(
			{
				...
				params: {
					...
					arguments: { projectId: "proj_1", locale: "fr", name: "greeting" },
				},
			},
			promptCtx,
		)) as {
		...
		expect(buildScorePrompt).toHaveBeenCalledWith(
			promptCtx.actor,
			"proj_1",
			"fr",
			{
		...
		const res = (await handleMessage(
			{
				...
				params: {
					name: "delete_key",
					arguments: { projectId: "p", name: "k", confirm: true },
				},
			},
			callCtx,
			{ mode: "classic", allowed: new Set(["get_key"]) },
		)) as { result: { isError?: boolean; content: { text: string }[] } };
		expect(res.result.isError).toBe(true);
	...
	it("does not expose the business toolset in code mode (mutually exclusive)", async () => {
		const res = (await handleMessage(
		...
		const res = (await handleMessage(
			{
				...
				params: {
					...
					arguments: { projectId: "proj_1", locale: "fr", entries },
				},
			},
			callCtx,
		...
		const res = (await handleMessage(
			{
				...
				params: {
					name: "delete_key",
					arguments: { projectId: "proj_1", name: "greeting" },
				},
			},
			callCtx,
		)) as { result?: { isError?: boolean }; error?: { code: number } };
		...
		const res = (await handleMessage(
			{
				...
				params: {
					...
					arguments: { projectId: "proj_1", locale: "fr", cursor: "c0" },
				},
			},
			callCtx,
