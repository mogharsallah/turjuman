import { type TurjumanService, type Repository, hashApiKey } from "@turjuman/core";
...
function fakeRepo(state: { touched: boolean }, opts: { readOnly?: boolean } = {}): Repository {
  return {
    getApiKeyByHash: async (hash: string) =>
      hash === hashApiKey(SECRET)
        ? { id: "key_1", orgId: "default", userId: user.id, name: "t", hash, prefix: "op_live_t", createdAt: "now", readOnly: opts.readOnly }
        : undefined,
    getUser: async (id: string) =>
      id === user.id ? { ...user, email: "t@t.com", name: "T", createdAt: "now", updatedAt: "now" } : undefined,
    touchApiKey: async () => {
...
describe("MCP handler — processRequest", () => {
  ...
  it("records the tool name on a tools/call", async () => {
    ...
    await processRequest({
      ...
      service: { repo: fakeRepo(state), service: { projects: { list: async () => [] } } as unknown as TurjumanService },
    });
    expect(lastLog(logSpy)).toMatchObject({ method: "tools/call", tool: "list_projects", outcome: "ok" });
  });
  ...
  it("rejects a bad key as unauthorized and still logs + stamps the response", async () => {
    ...
    expect(res.headers["x-request-id"]).toBeTruthy();
    expect(lastLog(logSpy)).toMatchObject({ outcome: "unauthorized", status: 401 });
  });
  ...
  it("honours an inbound x-request-id as the correlation id", async () => {
    ...
    const res = await processRequest({
      method: "POST",
      headers: { authorization: `Bearer ${SECRET}`, "x-request-id": "trace-xyz" },
      body: JSON.stringify({ jsonrpc: "2.0", id: 1, method: "tools/list" }),
      service: deps({ touched: false }),
    });
  ...
  it("rejects a present-but-unsupported MCP-Protocol-Version with 400", async () => {
    ...
    const res = await processRequest({
      method: "POST",
      headers: { authorization: `Bearer ${SECRET}`, "mcp-protocol-version": "1999-01-01" },
      body: JSON.stringify({ jsonrpc: "2.0", id: 1, method: "tools/list" }),
      service: deps({ touched: false }),
    });
    expect(res.statusCode).toBe(400);
    expect(lastLog(logSpy)).toMatchObject({ outcome: "unsupported_protocol_version", status: 400 });
  });
  ...
  it("scopes tools/list via the ?groups= query param", async () => {
    ...
    const tools = (JSON.parse(res.body ?? "{}").result.tools as { name: string }[]).map((t) => t.name);
  ...
  it("advertises only read tools to a read-only key (per-key filtering)", async () => {
    ...
    const res = await processRequest({
      ...
      headers: { authorization: `Bearer ${SECRET}` },
      body: JSON.stringify({ jsonrpc: "2.0", id: 1, method: "tools/list" }),
      service: { repo: fakeRepo({ touched: false }, { readOnly: true }), service: {} as TurjumanService },
    });
