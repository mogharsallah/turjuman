...
describe("resolveToolScope", () => {
	...
	it("groups delete_project under projects (decoupled from the source arrays)", () => {
		const allowed = (
			resolveToolScope({ groups: "projects" }) as { allowed: Set<string> }
		).allowed;
	...
	it("resolves the synthetic read group to read-only tools only", () => {
		const allowed = (
			resolveToolScope({ groups: "read" }) as { allowed: Set<string> }
		).allowed;
		expect(allowed).toContain("list_keys");
	...
	it("unions tools and groups", () => {
		const allowed = (
			resolveToolScope({ tools: "delete_key", groups: "qa" }) as {
				allowed: Set<string>;
			}
		).allowed;
		expect(allowed).toContain("delete_key");
	...
	it("fails loud on an unknown tool", () => {
		...
		expect(scope).toMatchObject({
			error: expect.stringContaining("nope_tool"),
		});
	});
...
describe("allowedToolsForActor", () => {
	...
	it("restricts a read-only key to read tools regardless of role", () => {
		const allowed = allowedToolsForActor(
			actor({ globalRole: "OWNER", readOnly: true }),
		);
		...
		expect(
			[...allowed].every(
				(n) => /^(list|get|search|lookup)_/.test(n) || n === "run_qa_checks",
			),
		).toBe(true);
	});
});
