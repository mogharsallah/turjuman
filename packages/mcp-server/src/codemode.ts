...
import { DEFAULT_LIMIT, KINDS, describeKnowledge, searchKnowledge } from "@turjuman/knowledge";
  ...
  op({
    ...
    input: z.object({
      query: z
        .string()
        .optional()
        .describe("Free-text search over operations and docs; omit for a cold-start orientation."),
    ...
    handler: (a) => searchKnowledge(a.query ?? "", { kind: a.kind, limit: a.limit }),
  }),
  op({
    name: "describe",
    description:
      ...
      "writing run_code.",
    input: z.object({
      id: z.string().describe('A result id from search, e.g. "op:set_translation" or "guides/code-mode.mdx#_intro".'),
    }),
    annotations: { readOnlyHint: true },
    handler: async (a) => {
