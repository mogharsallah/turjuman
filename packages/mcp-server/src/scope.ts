import { type Actor, canOnOrg, type OrgAction } from "@turjuman/core";
import {
	isReadOnly,
	OPERATION_GROUPS,
	OPERATIONS,
...
export function resolveToolScope(
	...
	const unknownTools = requestedTools.filter(
		(name) => !OPERATIONS_BY_NAME.has(name),
	);
	if (unknownTools.length > 0) {
		return { error: `Unknown tool(s): ${unknownTools.join(", ")}.` };
	}
	const unknownGroups = requestedGroups.filter(
		(g) => toolsInGroup(g) === undefined,
	);
