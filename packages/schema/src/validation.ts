...
export const keyNameSchema = z.string().trim().regex(KEY_NAME_RE, "Invalid key name");

/** A key namespace (logical group). Trimmed. */
export const namespaceSchema = z.string().trim().regex(NAMESPACE_RE, "Invalid namespace");

/** An email address. Trimmed and lower-cased. */
export const emailSchema = z.string().trim().toLowerCase().regex(EMAIL_RE, "Invalid email");
...
export const importKeysBodySchema = z.object({
...
export const runChecksBodySchema = z.object({
...
export function requireLocale(value: string, field: string): string {
  const v = (value ?? "").trim();
  if (!LOCALE_RE.test(v)) throw validation(`${field} must be a locale code like "en" or "es-MX"`);
  return v;
}

export function requirePattern(value: string, re: RegExp, field: string): string {
  const v = (value ?? "").trim();
