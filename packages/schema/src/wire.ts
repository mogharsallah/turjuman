...
export const apiKeyPublicSchema = apiKeySchema.omit({ hash: true }).openapi({ ref: "ApiKeyPublic" });
export type ApiKeyPublic = z.infer<typeof apiKeyPublicSchema>;

/** A webhook as listed/returned — never includes the signing secret. */
export const webhookPublicSchema = webhookSchema.omit({ secret: true }).openapi({ ref: "WebhookPublic" });
...
export const keyWithTranslationsSchema = z.object({
...
export const keyPageSchema = z.object({
  ...
  nextCursor: z.string().optional().describe("Opaque cursor; pass back as `cursor` for the next page."),
}).openapi({ ref: "KeyPage" });
...
export const importKeysResultSchema = z.object({
...
export const apiKeyCreatedSchema = z.object({
  id: z.string(),
  name: z.string(),
  prefix: z.string().describe('Leading characters of the secret, for recognition (e.g. "op_live_ab12").'),
  readOnly: z.boolean().describe("When true, the key may only perform read actions."),
  expiresAt: z.string().optional().describe("ISO-8601 expiry, if one was set."),
  secret: z.string().describe("The full secret — shown ONCE here; store it securely."),
...
export const qaFindingSchema = z.object({
  ...
  message: z.string(),
  value: z.string().optional().describe("A short snippet of the offending value."),
...
export const qaReportCountsSchema = z.object({
  error: z.number().int(),
...
export const qaReportSchema = z.object({
  ...
  checks: z.array(z.string()).describe("Check ids actually run (enabled ∩ requested)."),
  counts: qaReportCountsSchema,
  findings: z.array(qaFindingSchema).describe("Stable-sorted by locale, namespace, keyName, checkId."),
  byLocale: z.record(z.string(), z.array(qaFindingSchema)).describe("The same findings grouped by target locale."),
...
export const errorEnvelopeSchema = z.object({
  ...
  requestId: z.string().describe("Per-request correlation id, echoed in the X-Request-Id header."),
