...
import { scoreResultSchema, type ScoreContext } from "./types.js";

const ctx = (over: Partial<ScoreContext> = {}): ScoreContext => ({
  baseLocale: "en",
  targetLocale: "fr",
  key: { namespace: "default", name: "greeting", plural: false, description: "Home greeting", maxLength: 40 },
  baseValue: "Hello {name}",
  targetValue: "Bonjour {name}",
  glossary: [{ term: "Hello", translations: { fr: "Bonjour" }, doNotTranslate: false }],
  threshold: 90,
  ...over,
});
...
describe("scoring prompt", () => {
  ...
  it("renders a structured single-string prompt with every section + a strict JSON contract", () => {
    const { messages } = buildScorePrompt(ctx({ guidance: "Use vous, not tu." }));
    expect(messages).toHaveLength(1);
    const text = messages[0]!.text;
    for (const section of ["MQM rubric", "Project guidance", "Glossary", "Threshold", "Source (en)", "Target (fr) — grade THIS", "Output"]) {
  ...
  it("renders (none) for absent guidance/glossary", () => {
    const text = buildScorePrompt(ctx({ guidance: undefined, glossary: [] })).messages[0]!.text;
  ...
  it("renders a batch prompt keyed by namespace#name with a JSON-array contract", () => {
    const text = buildBatchScorePrompt([
      ctx(),
      ctx({ key: { namespace: "default", name: "farewell", plural: false }, baseValue: "Bye", targetValue: "Au revoir" }),
    ]).messages[0]!.text;
  ...
  it("the result contract accepts a valid score and rejects an out-of-range one", () => {
    expect(scoreResultSchema.safeParse({ score: 88, comment: "ok" }).success).toBe(true);
    expect(scoreResultSchema.safeParse({ score: 188, comment: "no" }).success).toBe(false);
  });
