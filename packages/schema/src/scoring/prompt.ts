...
import type { RenderedPrompt, ScoreContext, ScoreGlossaryTerm } from "./types.js";
...
function renderGlossary(glossary: readonly ScoreGlossaryTerm[], targetLocale: string): string {
  if (glossary.length === 0) return "(none)";
  return glossary
    .map((g) => {
      if (g.doNotTranslate) return `- "${g.term}" — DO NOT TRANSLATE (keep verbatim)`;
...
function renderKeyMeta(key: ScoreContext["key"]): string {
  const lines = [`namespace: ${key.namespace}`, `name: ${key.name}`, `plural: ${key.plural ? "yes" : "no"}`];
...
function preamble(ctx: Pick<ScoreContext, "baseLocale" | "targetLocale" | "glossary" | "guidance" | "threshold">): string {
  return [
 ...
 */
export function buildBatchScorePrompt(items: readonly ScoreContext[]): RenderedPrompt {
  if (items.length === 0) {
    return {
      messages: [
        { role: "user", text: "There are no translations to review on this page. Return an empty JSON array: []" },
      ],
    };
  }
  const head = items[0]!;
