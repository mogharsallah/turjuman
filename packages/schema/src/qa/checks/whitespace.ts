...
export const whitespaceCheck: QaCheck = {
  id: "whitespace",
  description: "Flag leading/trailing-whitespace differences vs base and double spaces.",
  severity: "warning",
  run(ctx) {
    ...
    if (base !== undefined) {
      if (leading(target) !== leading(base)) {
        issues.push(leading(target) ? "unexpected leading whitespace" : "missing leading whitespace");
      }
      if (trailing(target) !== trailing(base)) {
        issues.push(trailing(target) ? "unexpected trailing whitespace" : "missing trailing whitespace");
      }
    }
    if (/ {2,}/.test(target)) issues.push("contains consecutive spaces");
    if (issues.length === 0) return [];
    return [finding(ctx, this.id, this.severity, `Whitespace: ${issues.join("; ")}.`)];
  },
};
