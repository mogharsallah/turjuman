...
export const glossaryCheck: QaCheck = {
	id: "glossary",
	description:
		"Enforce do-not-translate terms and preferred per-locale translations.",
	severity: "error",
	run(ctx) {
		...
		for (const term of ctx.glossary as readonly GlossaryTerm[]) {
				...
				if (!contains(target, term.term, term.caseSensitive)) {
					out.push(
						finding(
							...
							this.id,
							"error",
							`Do-not-translate term "${term.term}" is missing from the target.`,
						),
					);
				}
				continue;
			...
			if (
				preferred &&
				preferred.trim() !== "" &&
				!contains(target, preferred, term.caseSensitive)
			) {
				out.push(
					finding(
						ctx,
