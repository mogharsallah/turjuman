			...
			if (!parsed) {
				return [
					finding(
						...
						this.id,
						this.severity,
						"Key is marked plural but the value is not an ICU plural message.",
					),
				];
			}
			if (parsed.forms.other === undefined) {
				return [
					finding(
						...
						this.id,
						this.severity,
						"Plural message is missing the required `other` form.",
					),
				];
			}
			return [];
		...
		if (parsed) {
			return [
				finding(
					...
					this.id,
					"warning",
					"Value is an ICU plural message but the key is not marked plural.",
				),
			];
		}
		return [];
