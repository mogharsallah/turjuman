...
export const placeholdersCheck: QaCheck = {
	id: "placeholders",
	description:
		"Target must reference the same variables/placeholders as the base value.",
	severity: "error",
	run(ctx) {
		...
		if (baseArgs === null) {
			return [
				finding(
					...
					this.id,
					"info",
					"Base value is not parseable ICU; placeholder check skipped.",
				),
			];
		}
		...
		return [
			finding(
				...
				this.id,
				this.severity,
				`Placeholder mismatch: ${parts.join("; ")}`,
			),
		];
	},
};
