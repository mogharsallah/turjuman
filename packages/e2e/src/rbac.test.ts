 ...
 */
const env = loadEnv();
const e = env ?? { mcpUrl: "", apiUrl: "", tableName: "", apiKey: "", apiKeyOrgB: undefined as string | undefined };
...
describe.skipIf(!env)("P1 security boundary", () => {
  ...
  it("enforces RBAC at the boundary: VIEWER reads, EDITOR writes", async () => {
    ...
    await ownerMcp("create_key", { projectId: project.id, name: "title", description: "Page title" });
    ...
    await expect(viewerMcp("get_project", { projectId: project.id })).resolves.toMatchObject({
      id: project.id,
    });
    const read = await viewerRest("GET", `v1/projects/${project.id}/translations?locale=fr`);
    ...
    const viewerWrite = await viewerRest("POST", `v1/projects/${project.id}/translations/import`, {
  ...
  it.skipIf(!e.apiKeyOrgB)("isolates tenants: another org cannot see this project", async () => {
    const project = await ownerMcp<{ id: string }>("create_project", {
      name: uniq("Tenant A"),
      baseLocale: "en",
    });
    ...
    await expect(orgBMcp("get_project", { projectId: project.id })).rejects.toThrow();
