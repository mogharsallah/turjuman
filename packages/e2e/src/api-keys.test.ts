 ...
 * teammate; the key authenticates at BOTH transports (MCP + REST); after
 * revocation the same key is rejected at both. Exercises the real
 * authenticate() path through the handlers.
 */
const env = loadEnv();
...
describe.skipIf(mode !== "inprocess")(
	...
	() => {
		...
		it("mints a working key, then revokes it and rejects it everywhere", async () => {
			...
			const key = await ownerMcp<{ id: string; secret: string }>(
				"create_api_key",
				{
					name: uniq("ci-token"),
					userId: user.id,
		...
		it("rejects a missing or garbage key at the REST boundary", async () => {
			...
			const garbage = await rest("GET", "v1/projects", {
				apiKey: "whsec_not-a-real-key",
			});
