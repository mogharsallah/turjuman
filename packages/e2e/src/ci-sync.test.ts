...
describe.skipIf(!env)("P1 developer CI sync (REST)", () => {
  ...
  it("imports keys + translations and exports them back over the Function URL", async () => {
    ...
    const keysImport = await rest<{ created: number }>(
      ...
      `v1/projects/${project.id}/keys/import`,
      {
        body: {
          entries: [
            { name: "nav.home", description: "Top nav home link", baseValue: "Home" },
            { name: "nav.about", description: "Top nav about link", baseValue: "About" },
          ],
        },
      },
    );
    ...
    const translations = await rest<{ locale: string; translations: Translation[] }>(
      "GET",
      `v1/projects/${project.id}/translations?locale=fr`,
    );
    expect(translations.status).toBe(200);
    expect(
      Object.fromEntries(translations.json.translations.map((t) => [t.keyName, t.value])),
    ...
    expect(Object.fromEntries(bundle.json.entries.map((b) => [b.key, b.value]))).toMatchObject({
      "nav.home": "Accueil",
      "nav.about": "À propos",
    });
  });
  ...
  it("keeps MCP and REST consistent: write via MCP, read via REST and back", async () => {
    ...
    // ...visible via REST.
    const viaRest = await rest<{ keys: Key[] }>("GET", `v1/projects/${project.id}/keys`);
    ...
    const restProjects = await rest<{ projects: { id: string }[] }>("GET", "v1/projects");
