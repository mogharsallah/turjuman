...
import { type SandboxRunResult, makeCodeClient, makeMcpClient, mcpListTools } from "./helpers/mcp.js";
...
const MODES: { name: string; run: Journey }[] = [
  {
    ...
    run: async (suffix) => {
      ...
      const project = await mcp<{ id: string }>("create_project", { name: suffix, baseLocale: "en" });
      await mcp("add_locale", { projectId: project.id, code: "fr" });
      await mcp("create_key", { projectId: project.id, name: "greeting", baseValue: "Hello" });
      await mcp("set_translation", { projectId: project.id, locale: "fr", name: "greeting", value: "Bonjour" });
      const translations = await mcp<{ localeCode: string; value: string; status: string }[]>("get_translations", {
...
describe.skipIf(!env)("P1 translate-then-read journey (classic vs code)", () => {
