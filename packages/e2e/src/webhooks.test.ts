...
const WAIT = 90_000;
const byEvent = (name: string) => (r: CapturedRequest) =>
	r.headers["x-turjuman-event"] === name;

function expectSigned(delivery: CapturedRequest, secret: string): void {
	const expected =
		"sha256=" +
		createHmac("sha256", secret).update(delivery.body).digest("hex");
	...
	it("delivers a distinct signed event for each change type", async () => {
		...
		try {
			...
			const [created, localeAdded, updated, deleted] = await Promise.all([
				...
				receiver.waitFor(
					(r) => byEvent("locale.added")(r) && r.body.includes('"code":"fr"'),
					WAIT,
				),
			...
			for (const d of [created, localeAdded, updated, deleted])
				expectSigned(d, webhook.secret);
