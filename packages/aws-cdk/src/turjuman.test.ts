...
describe("CDK synth", () => {
	...
	it("declares the single-table store with the expected schema, retained", () => {
		...
		expect(resourcesOfType(synthWith(), "AWS::DynamoDB::Table")[0]![0]).toBe(
			logicalId,
		);
		...
		expect(p.StreamSpecification).toEqual({
	...
	it("declares the three Lambda functions with the table env var and handlers", () => {
		...
		for (const [description, handler] of [
			...
			expect(fn!.Properties.Environment.Variables.TURJUMAN_TABLE).toEqual({
				Ref: tableId,
			});
		}
	});
	...
	it("outputs the three values the deployer reads back", () => {
		expect(Object.keys(synth.Outputs).sort()).toEqual([
			"ApiUrl",
			"McpUrl",
			"TableName",
		]);
	...
	it("drops the webhook function and its event source when webhook.enabled is false", () => {
		...
		expect(
			fnByDescription(t, "Turjuman webhook dispatcher (DynamoDB Streams)"),
		).toBeUndefined();
		expect(resourcesOfType(t, "AWS::Lambda::EventSourceMapping")).toHaveLength(
			0,
		);
	...
	it("switches the table (and its GSIs) to provisioned throughput on request", () => {
		const t = synthWith({
			table: { billingMode: "PROVISIONED", readCapacity: 5, writeCapacity: 7 },
		});
		const table = resourcesOfType(t, "AWS::DynamoDB::Table")[0]![1];
