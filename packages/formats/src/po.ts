...
export const poAdapter: FormatAdapter = {
  ...
  serialize(entries: TranslationEntry[], ctx?: FormatContext): string {
    ...
    for (const entry of sortByKey(entries)) {
      const plural = entry.plural || isIcuPlural(entry.value) ? parseIcuPlural(entry.value) : null;
  ...
  parse(content: string, ctx?: FormatContext): TranslationEntry[] {
    ...
    for (const group of Object.values(data.translations)) {
      for (const [msgid, t] of Object.entries(group)) {
        ...
        const description = t.comments?.extracted || t.comments?.translator || undefined;
        if (t.msgid_plural) {
          ...
          entries.push({ key: msgid, value: buildIcuPlural({ varName: "count", forms }), description, plural: true });
        } else {
          entries.push({ key: msgid, value: t.msgstr[0] ?? "", description, plural: false });
        }
      }
    }
    return entries;
