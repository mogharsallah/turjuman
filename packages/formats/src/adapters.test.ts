...
const SURVIVAL: Survival[] = [
  { id: "json-nested", keepsDescription: false, entries: "all", keepsPluralVarName: true },
  { id: "json-flat", keepsDescription: false, entries: "all", keepsPluralVarName: true },
  { id: "yaml", keepsDescription: false, entries: "all", keepsPluralVarName: true },
  { id: "arb", keepsDescription: true, entries: "all", keepsPluralVarName: true },
  { id: "properties", keepsDescription: true, entries: "all", keepsPluralVarName: true },
  { id: "csv", keepsDescription: true, entries: "all", keepsPluralVarName: true },
  // <!-- comment --> is write-only; <plurals> stores a category map (var name lost).
  { id: "android", keepsDescription: false, entries: "all", keepsPluralVarName: false },
  { id: "ios-strings", keepsDescription: true, entries: "singular", keepsPluralVarName: true }, // .strings has no plurals
  { id: "ios-stringsdict", keepsDescription: false, entries: "plural", keepsPluralVarName: true }, // plurals-only
  // gettext stores plurals by numeric index ⇒ var name reconstructed.
  { id: "po", keepsDescription: true, entries: "all", keepsPluralVarName: false },
  { id: "xliff-1.2", keepsDescription: true, entries: "all", keepsPluralVarName: true },
  { id: "xliff-2.0", keepsDescription: true, entries: "all", keepsPluralVarName: true },
];

const SURVIVAL_BY_ID = new Map(SURVIVAL.map((s) => [s.id, s]));

/** The set of input entries a spec says should survive the round-trip. */
function expectedSurvivors(entries: TranslationEntry[], spec: Survival): TranslationEntry[] {
 ...
 */
function assertSurvives(input: TranslationEntry[], output: TranslationEntry[], spec: Survival): void {
  ...
  for (const exp of expected) {
    ...
    const norm = (v: string) => (spec.keepsPluralVarName ? v : stripPluralVarName(v));
    expect(norm(actual!.value), `value for "${exp.key}"`).toBe(norm(exp.value));
    expect(Boolean(actual!.plural), `plural flag for "${exp.key}"`).toBe(Boolean(exp.plural));
    if (spec.keepsDescription) {
      expect(actual!.description, `description for "${exp.key}"`).toBe(exp.description);
    }
  }
}
...
describe("adapter registry ↔ survival spec", () => {
  it("every registered adapter has a field-survival spec (completeness ratchet)", () => {
    for (const a of ADAPTERS) {
      expect(SURVIVAL_BY_ID.has(a.id), `no survival spec for adapter "${a.id}"`).toBe(true);
    }
  });
  it("every spec names a real adapter", () => {
    const ids = new Set(ADAPTERS.map((a) => a.id));
    for (const s of SURVIVAL) expect(ids.has(s.id), `spec "${s.id}" is not a real adapter`).toBe(true);
  });
});
...
const ident = fc
  .array(fc.constantFrom(..."abcdefghijklmnopqrstuvwxyz0123456789".split("")), { minLength: 1, maxLength: 6 })
  .map((cs) => cs.join(""))
  .filter((s) => /^[a-z]/.test(s));

const dottedKey = fc.array(ident, { minLength: 1, maxLength: 3 }).map((p) => p.join("."));
...
const text = fc
  .array(fc.constantFrom(..."abcdefghijklmnopqrstuvwxyz ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789.,!?".split("")), {
    minLength: 1,
    maxLength: 24,
  })
  .map((cs) => cs.join(""))
...
const pluralValue = fc
  ...
  .map(({ v, one, other }) => buildIcuPlural({ varName: v, forms: { one, other } }));
      ...
      ...keys.map((key) =>
        fc
          ...
          .map(({ isPlural, text: t, pluralValue: pv, description }) => {
            const e: TranslationEntry = { key, value: isPlural ? pv : t, plural: isPlural };
...
describe("assertSurvives (self-test)", () => {
  const full: Survival = { id: "x", keepsDescription: true, entries: "all", keepsPluralVarName: true };
  it("passes an exact echo", () => {
    expect(() => assertSurvives(GOLDEN, GOLDEN, full)).not.toThrow();
  });
  it("fails when a value is mangled", () => {
    const broken = GOLDEN.map((e) => (e.key === singular.key ? { ...e, value: "WRONG" } : e));
  ...
  it("ignores description when the spec says it is not kept", () => {
    const noDesc: Survival = { id: "x", keepsDescription: false, entries: "all", keepsPluralVarName: true };
