...
export const iosStringsdictAdapter: FormatAdapter = {
	...
	serialize(entries: TranslationEntry[]): string {
		...
		return (
			plist.build(root as unknown as Parameters<typeof plist.build>[0]) + "\n"
		);
	},
	parse(content: string): TranslationEntry[] {
		...
		for (const [key, raw] of Object.entries(root)) {
			...
			for (const [subName, subRaw] of Object.entries(dict)) {
				...
				if (sub.NSStringFormatSpecTypeKey !== "NSStringPluralRuleType")
					continue;
				...
				entries.push({
					key,
					value: buildIcuPlural({ varName: subName, forms }),
					plural: true,
				});
				break;
			}
