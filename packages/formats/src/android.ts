...
export const androidAdapter: FormatAdapter = {
  ...
  serialize(entries: TranslationEntry[]): string {
    ...
    for (const entry of sortByKey(entries)) {
      if (entry.description) lines.push(`  <!-- ${escapeComment(entry.description)} -->`);
      ...
      if (plural) {
        ...
        lines.push(`  <string name="${entry.key}">${escapeText(entry.value)}</string>`);
      }
    }
  ...
  parse(content: string): TranslationEntry[] {
    ...
    const doc = parser.parse(content) as { resources?: Record<string, unknown> };
