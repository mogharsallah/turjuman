...
import type { Item, IndexName } from "./item.js";
import {
  apiKeyPK,
  emailPK,
  glossarySK,
...
export class Repository {
  ...
  async createUser(user: User): Promise<User> {
    try {
      ...
      if (isConditionalFailure(err)) throw conflict(`Email ${user.email} is already in use`);
      throw err;
    }
    return user;
  }
  ...
  async getUserByEmail(email: string): Promise<User | undefined> {
    const userId = await this.getItem(emailPK(email), emailPK(email), (i) => i.userId as string);
  ...
  async updateProject(
    ...
    if (patch.name !== undefined) (sets.push("#n = :n"), (values[":n"] = patch.name));
    ...
    await this.doc.send(
      new UpdateCommand({
        ...
        ExpressionAttributeNames: patch.name !== undefined ? { "#n": "name" } : undefined,
        ConditionExpression: "attribute_exists(PK)",
      }),
    );
  }
  ...
  async deleteKey(projectId: string, namespace: string, name: string): Promise<void> {
   ...
   */
  async deleteKeysCascade(projectId: string, namespace: string, names: string[]): Promise<void> {
    const translationLists = await Promise.all(
      names.map((name) => this.listTranslationsByKey(projectId, namespace, name)),
    );
    const toDelete: { PK: string; SK: string }[] = [];
    for (let i = 0; i < names.length; i++) {
      const name = names[i]!;
      for (const t of translationLists[i]!) {
        toDelete.push({ PK: transPK(projectId, t.localeCode), SK: keySK(namespace, name) });
      }
  ...
  async getGlossaryTerm(projectId: string, termId: string): Promise<GlossaryTerm | undefined> {
    return this.getItem(projectPK(projectId), glossarySK(termId), toGlossaryTerm);
  }
  ...
  async getWebhook(projectId: string, id: string): Promise<Webhook | undefined> {
   ...
   */
  async deleteProjectCascade(projectId: string, localeCodes: string[]): Promise<void> {
    const toDelete: { PK: string; SK: string }[] = [];
    ...
    for (const code of localeCodes) {
      ...
      for (const item of translations) toDelete.push({ PK: item.PK, SK: item.SK });
    }
    await this.batchDelete(toDelete);
  }
    ...
    for (let i = 0; i < list.length; i += 25) {
      ...
      await this.doc.send(
        new BatchWriteCommand({
          RequestItems: {
            [this.table]: chunk.map((t) => ({ PutRequest: { Item: translationItem(t) } })),
          },
        }),
      );
    }
  ...
  async getTranslation(
    ...
    return this.getItem(transPK(projectId, code), keySK(namespace, name), toTranslation);
  }
  ...
  async listTranslationsByLocalePage(
    ...
    const res = await this.doc.send(
      new QueryCommand({
        TableName: this.table,
        KeyConditionExpression: "PK = :p AND begins_with(SK, :s)",
        ExpressionAttributeValues: { ":p": transPK(projectId, code), ":s": "KEY#" },
        Limit: opts.limit,
        ExclusiveStartKey: decodeCursor(opts.cursor),
      }),
    );
    return {
      translations: ((res.Items as Item[] | undefined) ?? []).map(toTranslation),
      nextCursor: encodeCursor(res.LastEvaluatedKey as Record<string, unknown> | undefined),
    };
  }
  ...
  async listTranslationsByKey(
    ...
    const items = await this.queryAll({
      ...
      ExpressionAttributeValues: { ":p": keyGSI3PK(projectId, namespace, name) },
    });
