...
import type { User } from "@turjuman/schema";
 ...
 */
export function newApiKeySecret(): { secret: string; prefix: string; hash: string } {
...
export async function authenticate(
  ...
  if (apiKey.expiresAt && Date.parse(apiKey.expiresAt) <= Date.now()) return undefined;
