...
import type { KeyPage, ReviewResult, ScoreInput, ScorePrompt, SetScoreConfigInput } from "./types.js";
...
export class ScoringService extends BaseService {
  ...
  async getConfig(actor: Actor, projectId: string): Promise<ScoreConfig> {
    ...
    return (await this.repo.getScoreConfig(projectId)) ?? this.defaultConfig(projectId);
  }

  async setConfig(actor: Actor, projectId: string, input: SetScoreConfigInput): Promise<ScoreConfig> {
    await this.authorizeProject(actor, projectId, "project.update");
    if (input.threshold !== undefined) this.assertScore(input.threshold, "threshold");
    const existing = (await this.repo.getScoreConfig(projectId)) ?? this.defaultConfig(projectId);
    const merged: ScoreConfig = {
      projectId,
      threshold: input.threshold ?? existing.threshold,
      autoApprove: input.autoApprove ?? existing.autoApprove,
      guidance: input.guidance !== undefined ? input.guidance : existing.guidance,
  ...
  async score(
    ...
    const { project, role } = await this.authorizeProject(actor, projectId, "translation.write");
    ...
    if (!existing) throw notFound(`No ${code} translation for ${namespace}/${input.name}`);
    if (!this.hasValue(existing)) {
      throw validation(`Cannot score ${namespace}/${input.name} in ${code}: it has no translated value yet`);
    }
    const config = (await this.repo.getScoreConfig(projectId)) ?? this.defaultConfig(projectId);
    return this.repo.putTranslation(this.applyScore(existing, input, config, actor, role));
  }
  ...
  async reviewBatch(
    ...
    for (const e of entries) byId.set(`${e.namespace ?? DEFAULT_NAMESPACE}#${e.name}`, e);
    ...
    for (const [id, e] of byId) {
      ...
      if (next.status === "approved" && existing.status !== "approved") approved++;
  ...
  async buildScorePrompt(
    ...
    if ("name" in sel) {
      ...
      const [target, key, base] = await Promise.all([
        ...
        this.repo.getTranslation(projectId, project.baseLocale, namespace, sel.name),
      ]);
      if (!key) throw notFound(`Key ${namespace}/${sel.name} not found`);
      if (!target) throw notFound(`No ${code} translation for ${namespace}/${sel.name}`);
      const ctx = this.toScoreContext(project, code, key, base?.value ?? "", target.value, glossary, config);
    ...
    const baseById = new Map(baseTranslations.map((t) => [`${t.namespace}#${t.keyName}`, t.value]));
    ...
    for (const t of page.translations) {
      ...
      ctxs.push(this.toScoreContext(project, code, key, baseById.get(id) ?? "", t.value, glossary, config));
    }
    return { messages: buildBatchScorePrompt(ctxs).messages, promptVersion, nextCursor: page.nextCursor };
  }

  // ---- internals ------------------------------------------------------------

  private defaultConfig(projectId: string): ScoreConfig {
    return { projectId, threshold: 90, autoApprove: false, updatedBy: "", updatedAt: "" };
  }
  ...
  private promptVersion(config: ScoreConfig): string {
    return config.guidance?.trim() ? `${SCORE_PROMPT_VERSION}+custom` : SCORE_PROMPT_VERSION;
  }
