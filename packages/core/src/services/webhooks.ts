...
import type { Webhook } from "@turjuman/schema";
...
export class WebhooksService extends BaseService {
  ...
  async add(actor: Actor, projectId: string, input: AddWebhookInput): Promise<Webhook> {
    await this.authorizeProject(actor, projectId, "webhook.manage");
    const url = (input.url ?? "").trim();
    if (!/^https?:\/\/.+/.test(url)) throw validation("Webhook url must be http(s)");
