import { DEFAULT_NAMESPACE, type Translation, type TranslationKey } from "@turjuman/schema";
import { conflict, notFound, validation } from "@turjuman/schema";
import type { Actor } from "@turjuman/schema";
import { KEY_NAME_RE, NAMESPACE_RE, requirePattern } from "@turjuman/schema";
import type { ImportKeysResult } from "@turjuman/schema";
import { BaseService } from "./base.js";
import type { CreateKeyInput, KeyPage, KeyWithTranslations, UpdateKeyInput } from "./types.js";

export class KeysService extends BaseService {
  async list(
    actor: Actor,
    projectId: string,
    filter: { namespace?: string; tag?: string; includeDeprecated?: boolean } = {},
  ): Promise<TranslationKey[]> {
    await this.authorizeProject(actor, projectId, "key.read");
    let keys = await this.repo.listKeys(projectId, filter.namespace);
    if (!filter.includeDeprecated) keys = keys.filter((k) => k.state !== "deprecated");
  ...
  async listPage(
    ...
    let keys = page.keys;
    if (!opts.includeDeprecated) keys = keys.filter((k) => k.state !== "deprecated");
    if (opts.tag) keys = keys.filter((k) => k.tags.includes(opts.tag!));
    return { keys, nextCursor: page.nextCursor };
  }

  async search(actor: Actor, projectId: string, query: string): Promise<TranslationKey[]> {
  ...
  async searchPage(
    ...
    const q = query.toLowerCase();
    const keys = page.keys.filter((k) => k.state !== "deprecated" && this.matchesQuery(k, q));
    return { keys, nextCursor: page.nextCursor };
  }
  ...
  async get(
    ...
    const translations = await this.repo.listTranslationsByKey(projectId, namespace, name);
    return { key, translations };
  }

  async create(actor: Actor, projectId: string, input: CreateKeyInput): Promise<TranslationKey> {
    const { project } = await this.authorizeProject(actor, projectId, "key.manage");
    const namespace = requirePattern(input.namespace ?? DEFAULT_NAMESPACE, NAMESPACE_RE, "namespace");
  ...
  async import(
    actor: Actor,
    projectId: string,
    entries: { name: string; description?: string; baseValue?: string; plural?: boolean }[],
    namespace = DEFAULT_NAMESPACE,
    ...
    if (opts.prune) {
      ...
      if (names.length > 0) await this.repo.deleteKeysCascade(projectId, ns, names);
  ...
  async delete(
    ...
    await this.authorizeProject(actor, projectId, "key.manage");
    if (!confirm) {
      throw validation("Set confirm=true to permanently delete the key and all its translations");
    }
