...
import { newId, slugify } from "@turjuman/schema";
import { type Actor, requireOrg } from "@turjuman/schema";
import { requireLocale, requireText } from "@turjuman/schema";
...
export class ProjectsService extends BaseService {
  async list(actor: Actor): Promise<Project[]> {
    ...
    const projects = await Promise.all(memberships.map((m) => this.repo.getProject(m.projectId)));
    return projects.filter((p): p is Project => !!p && p.orgId === actor.orgId);
  }

  async get(actor: Actor, projectId: string): Promise<Project> {
    const { project } = await this.authorizeProject(actor, projectId, "project.read");
    return project;
  }
  ...
  async update(
    ...
    if (patch.baseLocale !== undefined) patch.baseLocale = requireLocale(patch.baseLocale, "baseLocale");
  ...
  async delete(
    ...
    if (!confirm) {
      throw validation("Set confirm=true to permanently delete the project and all its data");
    }
