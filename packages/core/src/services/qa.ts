import type { Project, QaConfig, QaIgnoreRule, QaSeverity } from "@turjuman/schema";
...
import type { QaContext, QaFinding, QaReport, QaReportCounts } from "@turjuman/schema/qa";
...
export class QaService extends BaseService {
  async getConfig(actor: Actor, projectId: string): Promise<QaConfig> {
    ...
    return (await this.repo.getQaConfig(projectId)) ?? this.defaultConfig(projectId);
  }

  async setConfig(actor: Actor, projectId: string, input: SetQaConfigInput): Promise<QaConfig> {
    ...
    for (const rule of input.ignore ?? []) {
      ...
      if (!rule.checkId && !rule.namespace && !rule.keyName && !rule.locale) {
        throw validation("An ignore rule must specify at least one of checkId/namespace/keyName/locale");
      }
    }
    const existing = (await this.repo.getQaConfig(projectId)) ?? this.defaultConfig(projectId);
  ...
  async run(actor: Actor, projectId: string, opts: RunChecksOptions = {}): Promise<QaReport> {
    ...
    const config = (await this.repo.getQaConfig(projectId)) ?? this.defaultConfig(projectId);
    ...
    return { projectId, baseLocale: project.baseLocale, locales, checks: checkIds, counts, findings, byLocale };
  }
  ...
  private enabledCheckIds(config: QaConfig): string[] {
    return CHECKS.filter(
      (c) => config.checks[c.id]?.enabled ?? !DEFAULT_DISABLED_CHECKS.includes(c.id),
    ).map((c) => c.id);
  }
  ...
  private applyConfig(findings: QaFinding[], config: QaConfig): QaFinding[] {
    return findings
      ...
      .filter((f) => !config.ignore.some((rule) => this.matchesIgnore(rule, f)));
  }
  ...
  private async buildContexts(
    ...
    for (const code of locales) {
      const translations = await this.repo.listTranslationsByLocale(projectId, code);
      const byKey = new Map(translations.map((t) => [`${t.namespace}#${t.keyName}`, t]));
      const valueOf = (t: (typeof translations)[number] | undefined): string | undefined =>
