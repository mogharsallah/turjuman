...
class FakeRepo {
  ...
  keys = new Map<string, TranslationKey>(); // `${projectId}#${ns}#${name}`
  translations = new Map<string, Translation>(); // `${projectId}#${code}#${ns}#${name}`
  glossary = new Map<string, GlossaryTerm[]>();
  scoreConfigs = new Map<string, ScoreConfig>();
  ...
  async getLocale(projectId: string, code: string) {
  ...
  async getKey(projectId: string, ns: string, name: string) {
    return this.keys.get(`${projectId}#${ns}#${name}`);
  }
  async listKeys(projectId: string, namespace?: string) {
    const all = [...this.keys.values()]
      .filter((k) => k.projectId === projectId && (!namespace || k.namespace === namespace))
      .sort((a, b) => `${a.namespace}#${a.name}`.localeCompare(`${b.namespace}#${b.name}`));
    return all;
  }
  async listKeysPage(projectId: string, opts: { namespace?: string; limit?: number; cursor?: string } = {}) {
  ...
  async getTranslation(projectId: string, code: string, ns: string, name: string) {
    return this.translations.get(`${projectId}#${code}#${ns}#${name}`);
  }
  async putTranslation(t: Translation) {
    this.translations.set(`${t.projectId}#${t.localeCode}#${t.namespace}#${t.keyName}`, t);
    return t;
  }
  async putTranslations(list: Translation[]) {
    for (const t of list) await this.putTranslation(t);
  }
  async listTranslationsByLocale(projectId: string, code: string) {
    return [...this.translations.values()].filter((t) => t.projectId === projectId && t.localeCode === code);
  }
  ...
  async listGlossary(projectId: string) {
    return this.glossary.get(projectId) ?? [];
  }
  async getScoreConfig(projectId: string) {
    return this.scoreConfigs.get(projectId);
  }
  async putScoreConfig(config: ScoreConfig) {
...
function seed(opts: SeedOpts = {}) {
  ...
  const svc = new ScoringService(repo as unknown as Repository);
  ...
  repo.keys.set(`${projectId}#default#greeting`, {
    projectId,
    namespace: "default",
    name: "greeting",
    plural: false,
  ...
  repo.translations.set(`${projectId}#en#default#greeting`, {
    projectId,
    localeCode: "en",
    namespace: "default",
    keyName: "greeting",
  ...
  repo.translations.set(`${projectId}#fr#default#greeting`, {
    projectId,
    localeCode: "fr",
    namespace: "default",
    keyName: "greeting",
...
describe("ScoringService routing matrix", () => {
  it("a score below the threshold flags needs_review and stamps provenance", async () => {
    const { svc, projectId, owner } = seed({ threshold: 90, autoApprove: true });
  ...
  it("a high score auto-approves machine work when opted in and the actor can review", async () => {
    const { svc, projectId, owner } = seed({ threshold: 90, autoApprove: true, origin: "llm" });
  ...
  it("a high score does NOT auto-approve when the actor lacks the review permission", async () => {
    const { svc, projectId, setRole } = seed({ threshold: 90, autoApprove: true });
  ...
  it("a high score does NOT auto-approve when auto-approve is off", async () => {
    const { svc, projectId, owner } = seed({ threshold: 90, autoApprove: false });
    const t = await svc.score(owner, projectId, "fr", { name: "greeting", score: 95 });
    expect(t.status).toBe("translated");
  });

  it("a high score NEVER auto-approves human-origin work", async () => {
    const { svc, projectId, owner } = seed({ threshold: 90, autoApprove: true, origin: "human" });
  ...
  it("uses the default config (threshold 90, auto-approve off) when none is set", async () => {
    const { svc, projectId, owner } = seed(); // no config row
...
describe("ScoringService provenance + config", () => {
  it("stamps a +custom prompt version when the project sets evaluation guidance", async () => {
    const { svc, projectId, owner } = seed({ guidance: "Prefer formal register." });
  ...
  it("getConfig returns defaults; setConfig persists and round-trips", async () => {
    const { svc, projectId, owner } = seed();
  ...
  it("a re-score overwrites the prior score (no history)", async () => {
    const { svc, projectId, owner } = seed({ threshold: 90 });
...
describe("ScoringService prompt assembly", () => {
  it("assembles a prompt with the rubric, guidance, glossary, source and target", async () => {
    const { repo, svc, projectId, owner } = seed({ guidance: "Use vous, not tu." });
...
describe("ScoringService guards", () => {
  it("refuses to score a translation that has no value yet", async () => {
    const { svc, projectId, owner } = seed({ value: "" });
    await expect(
  ...
  it("reviewBatch skips entries whose translation has no value", async () => {
    const { repo, svc, projectId, owner } = seed({ threshold: 90, autoApprove: true });
    ...
    repo.keys.set(`${projectId}#default#blank`, {
      projectId,
      namespace: "default",
      name: "blank",
      plural: false,
...
describe("needs_review and the QA engine", () => {
  it("treats a needs_review value as expected (the empty check fires when it is blank)", async () => {
    ...
    const qa = new QaService(repo as unknown as Repository);
...
describe("ScoringService authorization", () => {
  it("denies scoring to a VIEWER (no translation.write)", async () => {
    const { svc, projectId, setRole } = seed();
