...
export abstract class BaseService {
  ...
  protected async authorizeProject(
    ...
    if (!project || project.orgId !== actor.orgId) throw notFound("Project not found");
    requireProject(actor, action, role);
    return { project, role };
  }

  /** Effective project role, short-circuiting the DB read for OWNER/ADMIN. */
  protected async projectRole(actor: Actor, projectId: string): Promise<ProjectRole | undefined> {
    if (actor.globalRole === "OWNER" || actor.globalRole === "ADMIN") return "MANAGER";
    return (await this.repo.getMembership(projectId, actor.userId))?.role;
  }

  protected async requireLocaleExists(projectId: string, code: string): Promise<void> {
    if (!(await this.repo.getLocale(projectId, code))) {
      throw notFound(`Locale ${code} does not exist on this project`);
    }
  }

  protected async findUser(ref: { userId?: string; email?: string }): Promise<User | undefined> {
    return ref.userId
