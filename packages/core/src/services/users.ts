...
import { type Actor, conflict, notFound, requireOrg } from "@turjuman/schema";
import { BaseService } from "./base.js";

/** OWNER/ADMIN are privileged: granting or touching them needs `role.admin` (OWNER-only). */
const isPrivileged = (role: GlobalRole): boolean =>
...
export class UsersService extends BaseService {
	...
	async create(
		...
		if (input.globalRole && isPrivileged(input.globalRole))
			requireOrg(actor, "role.admin");
	...
	async setGlobalRole(
		...
		if (isPrivileged(user.globalRole) || isPrivileged(role))
			requireOrg(actor, "role.admin");
