 ...
 */
const compareSk = (a: string, b: string): number => (a < b ? -1 : a > b ? 1 : 0);
...
export class FakeRepo implements RepositoryApi {
  ...
  async createUser(u: User): Promise<User> {
    ...
    if (this.emails.has(emailKey)) throw conflict(`Email ${u.email} is already in use`);
  ...
  async setUserGlobalRole(userId: string, role: GlobalRole): Promise<void> {
    ...
    if (u) this.users.set(userId, { ...u, globalRole: role, updatedAt: new Date().toISOString() });
  }
  ...
  async updateProject(
    ...
    if (!p) return;
    this.projects.set(projectId, { ...p, ...patch, updatedAt: new Date().toISOString() });
  }
  ...
  async listMembersByProject(projectId: string): Promise<Membership[]> {
    return [...this.memberships.values()].filter((m) => m.projectId === projectId);
  }
  ...
  async deleteProjectCascade(projectId: string, localeCodes: string[]): Promise<void> {
    ...
    for (const [k] of this.locales) if (k.startsWith(`${projectId}#`)) this.locales.delete(k);
    for (const [k, m] of this.memberships) if (m.projectId === projectId) this.memberships.delete(k);
    const trans = this.translations.get(projectId);
    if (trans) {
      for (const [k, t] of trans) if (localeCodes.includes(t.localeCode)) trans.delete(k);
    }
  }
