...
interface DeniedCase {
  ...
  call: (svc: TurjumanService, ro: Actor, projectId: string) => Promise<unknown>;
}
...
const DENIED: DeniedCase[] = [
  // ---- org-scoped --------------------------------------------------------------
  { op: "create_project", action: "project.create", call: (s, ro) => s.projects.create(ro, { name: "Nope", baseLocale: "en" }) },
  { op: "create_user", action: "user.manage", call: (s, ro) => s.users.create(ro, { email: "x@acme.com", name: "X" }) },
  { op: "set_user_role", action: "user.manage", call: (s, ro) => s.users.setGlobalRole(ro, "user_x", "MEMBER") },
  // ---- project lifecycle -------------------------------------------------------
  { op: "update_project", action: "project.update", call: (s, ro, p) => s.projects.update(ro, p, { name: "X" }) },
  { op: "delete_project", action: "project.delete", call: (s, ro, p) => s.projects.delete(ro, p, true) },
  // ---- locales -----------------------------------------------------------------
  { op: "add_locale", action: "locale.manage", call: (s, ro, p) => s.locales.add(ro, p, "de") },
  // ---- keys --------------------------------------------------------------------
  { op: "create_key", action: "key.manage", call: (s, ro, p) => s.keys.create(ro, p, { name: "k" }) },
  { op: "update_key", action: "key.manage", call: (s, ro, p) => s.keys.update(ro, p, "k", { description: "d" }) },
  { op: "delete_key", action: "key.manage", call: (s, ro, p) => s.keys.delete(ro, p, "k", true) },
  { op: "import_keys (CLI)", action: "key.manage", call: (s, ro, p) => s.keys.import(ro, p, [{ name: "k" }]) },
  // ---- translations ------------------------------------------------------------
  { op: "set_translation", action: "translation.write", call: (s, ro, p) => s.translations.set(ro, p, "en", { name: "k", value: "v" }) },
  { op: "bulk_set_translations", action: "translation.write", call: (s, ro, p) => s.translations.bulkSet(ro, p, "en", [{ name: "k", value: "v" }]) },
  { op: "set_translation_status", action: "translation.review", call: (s, ro, p) => s.translations.setStatus(ro, p, "en", "k", "approved") },
  // ---- glossary ----------------------------------------------------------------
  { op: "add_glossary_term", action: "glossary.manage", call: (s, ro, p) => s.glossary.add(ro, p, { term: "t" }) },
  { op: "update_glossary_term", action: "glossary.manage", call: (s, ro, p) => s.glossary.update(ro, p, "tid", { notes: "n" }) },
  { op: "remove_glossary_term", action: "glossary.manage", call: (s, ro, p) => s.glossary.remove(ro, p, "tid") },
  // ---- webhooks ----------------------------------------------------------------
  { op: "add_webhook", action: "webhook.manage", call: (s, ro, p) => s.webhooks.add(ro, p, { url: "https://e.co/h", events: ["key.created"] }) },
  { op: "remove_webhook", action: "webhook.manage", call: (s, ro, p) => s.webhooks.remove(ro, p, "wid") },
  // ---- members -----------------------------------------------------------------
  { op: "add_member", action: "member.manage", call: (s, ro, p) => s.members.add(ro, p, { userId: "user_x" }, "EDITOR") },
  { op: "set_member_role", action: "member.manage", call: (s, ro, p) => s.members.setRole(ro, p, "user_x", "EDITOR") },
  { op: "remove_member", action: "member.manage", call: (s, ro, p) => s.members.remove(ro, p, "user_x") },
  // ---- QA + scoring config -----------------------------------------------------
  { op: "set_qa_config", action: "project.update", call: (s, ro, p) => s.qa.setConfig(ro, p, { checks: { empty: { enabled: false } } }) },
  { op: "set_score_config", action: "project.update", call: (s, ro, p) => s.scoring.setConfig(ro, p, { threshold: 50 }) },
  { op: "score_translation", action: "translation.write", call: (s, ro, p) => s.scoring.score(ro, p, "en", { name: "k", score: 50 }) },
  { op: "review_translations", action: "translation.write", call: (s, ro, p) => s.scoring.reviewBatch(ro, p, "en", [{ name: "k", score: 50 }]) },
];
...
describe("RBAC service seam — a read-only key is denied every mutation", () => {
  describe.each(DENIED)("$op ($action)", ({ call }) => {
    it("rejects with FORBIDDEN", async () => {
      ...
      await expect(call(svc, ro, projectId)).rejects.toMatchObject({ code: "FORBIDDEN" });
    });
  });
  ...
  it("the read-only probe can still READ the project (so denials mean what they say)", async () => {
    const { svc, ro, projectId } = await readOnlySetup();
    await expect(svc.projects.get(ro, projectId)).resolves.toMatchObject({ id: projectId });
