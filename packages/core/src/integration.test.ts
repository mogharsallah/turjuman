...
import { type Actor, AppError } from "@turjuman/schema";
...
describe.skipIf(!endpoint)("end-to-end against DynamoDB", () => {
	...
	it("runs the full translation lifecycle with RBAC and tenant isolation", async () => {
		...
		const greetingTranslations = await svc.translations.listForKey(
			owner,
			project.id,
			"greeting",
		);
		expect(greetingTranslations.map((t) => t.localeCode).sort()).toEqual([
			"en",
			"fr",
		]);

		// --- review workflow: approving promotes the working value into approvedValue ---
		const approved = await svc.translations.setStatus(
			owner,
			project.id,
			"fr",
			"greeting",
		...
		const enExport = await svc.translations.listForLocale(
			owner,
			project.id,
			"en",
		);
		expect(enExport.map((t) => t.keyName).sort()).toEqual([
			"app.title",
			"greeting",
			"nav.home",
		]);
		...
		const doomed = await svc.projects.create(owner, {
			name: "Doomed",
			baseLocale: "en",
		});
		await svc.locales.add(owner, doomed.id, "fr");
		await svc.keys.create(owner, doomed.id, {
			name: "temp.key",
			baseValue: "x",
		});
		...
		expect(await repo.listTranslationsByLocale(doomed.id, "en")).toHaveLength(
			0,
		);
		...
		const pruneProject = await svc.projects.create(owner, {
			name: "Prune",
			baseLocale: "en",
		});
		...
		expect((await repo.listKeys(pruneProject.id)).map((k) => k.name)).toEqual([
			"keep.me",
		]);
		...
		let pages = 0;
		do {
			const page = await svc.keys.listPage(owner, pruneProject.id, {
				limit: 2,
				cursor,
			});
		...
		await expect(
			svc.projects.get(otherActor, project.id),
		).rejects.toMatchObject({ code: "NOT_FOUND" });
	}, 60_000);
});
...
describe.skipIf(!endpoint)("single-table invariants (DynamoDB)", () => {
	...
	it("partitions GSI1 by org and fans out GSI3 by key", async () => {
		...
		await svc.keys.create(owner, mine.id, {
			name: "greeting",
			baseValue: "Hello",
		});
		...
		const byKey = await repo.listTranslationsByKey(
			mine.id,
			"default",
			"greeting",
		);
