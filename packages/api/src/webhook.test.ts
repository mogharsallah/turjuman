...
import { TurjumanService, Repository, authenticate, bootstrapOwner } from "@turjuman/core";
...
describe.skipIf(!endpoint)("webhook dispatcher", () => {
  ...
  it("delivers an HMAC-signed POST for a matching event", async () => {
    ...
    const project = await svc.projects.create(actor, { name: "Hooked", baseLocale: "en" });
    ...
    await handler({ Records: [{ eventName: "MODIFY", dynamodb: { NewImage } }] });
