...
const DELIVERY_TIMEOUT_MS =
	Number(process.env.TURJUMAN_WEBHOOK_TIMEOUT_MS) || 5000;
...
export async function handler(event: {
	...
	const eventCount = [...byProject.values()].reduce(
	...
	await Promise.all(
		[...byProject].map(([projectId, events]) => dispatch(projectId, events)),
	);
}
...
async function baseLocaleFor(
	...
	if (!cache.has(projectId))
		cache.set(projectId, (await repo.getProject(projectId))?.baseLocale);
	...
	await Promise.all(
		webhooks.flatMap((webhook) =>
			events
				.filter(
					(e) =>
						webhook.events.includes("*") || webhook.events.includes(e.event),
				)
