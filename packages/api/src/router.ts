...
function queryLimit(raw: string | undefined): number | undefined {
	...
	if (!Number.isInteger(n) || n <= 0)
		throw validation("limit must be a positive integer");
	return n;
}
...
export function createApp(deps: RouterDeps): Hono<Env> {
	...
	app.onError((err, c) => {
		...
		if (masked.isAppError) {
			...
			accessLog(c, status);
			return c.json(
				{ error: masked.message, code: masked.code, requestId },
	...
	app.get(
		...
		describeRoute({
			...
			description:
				"Returns the service name and version. Public — no auth required.",
			tags: ["Meta"],
			security: [],
			responses: {
				200: jsonResponse(
					serviceMetaResponseSchema,
	...
	app.get(
		...
		describeRoute({
			...
			description:
				"The locale file formats the CLI can read/write. Public — no auth required.",
			tags: ["Meta"],
			security: [],
			responses: {
				200: jsonResponse(formatListResponseSchema, "Supported formats"),
			},
		}),
	...
	const projectOperation = (name: string, cfg: ProjectionConfig): void => {
		...
		if (!op?.http || !op.output) {
			throw new Error(
				`Operation "${name}" has no http binding or output schema to project`,
			);
		}
		...
		if (path !== PROJECTS_PREFIX && !path.startsWith(`${PROJECTS_PREFIX}/`)) {
			throw new Error(
				`Operation "${name}" path "${path}" is not under ${PROJECTS_PREFIX}; cannot project it onto the projects router`,
			);
		}
		...
		const spec = describeRoute({
			...
			tags: cfg.tags,
			responses: {
				[status]: jsonResponse(op.output, cfg.responseDescription),
				...errorResponses,
			},
		});
		const handler = async (c: Context<Env>) => {
			...
			for (const p of paramSources) {
				...
				const raw = p.fromPath
					? c.req.param(p.urlName)
					: c.req.query(p.urlName);
				if (raw !== undefined) merged[p.field] = raw;
			}
			if (cfg.body)
				Object.assign(
					merged,
					(c.req.valid as (t: "json") => Record<string, unknown>)("json"),
				);
			...
			try {
				...
				throw validation(
					e instanceof z.ZodError
						? (e.errors[0]?.message ?? "Invalid request")
						: "Invalid request",
				);
			}
		...
		if (cfg.body) {
			projects.on(
				...
				subPath,
				spec,
				validator("json", cfg.body, onInvalid),
				handler,
			);
		} else {
	...
	projects.get(
		...
		async (c) =>
			c.json({
				locales: await svc.locales.list(c.get("actor"), c.req.param("id")),
			}),
	);
	...
	projects.get(
		...
		describeRoute({
			...
			parameters: [
				{
					...
					name: "namespace",
					schema: { type: "string" },
					description: "Filter to a namespace",
				},
				{
					in: "query",
					name: "limit",
			...
			responses: {
				200: jsonResponse(
					keyPageSchema,
					"Keys (and nextCursor when paginated)",
				),
				...errorResponses,
			},
		}),
	...
	projects.post(
		...
		describeRoute({
			...
			responses: {
				200: jsonResponse(
					importKeysResultSchema,
					"Import summary (counts of what changed)",
				),
				...errorResponses,
			},
		}),
	...
	projects.get(
		...
		describeRoute({
			...
			description:
				"All translations for a locale; paginate with limit + cursor.",
			tags: ["Translations"],
			parameters: [
				{
					in: "query",
			...
			responses: {
				200: jsonResponse(
					localeTranslationsResponseSchema,
					"Translations (and nextCursor when paginated)",
				),
				...errorResponses,
			},
		}),
		...
		describeRoute({
			...
			parameters: [
				...
				},
				{
					in: "query",
					name: "fallback",
					schema: { type: "string", enum: ["omit"] },
					description:
						"omit = drop keys with no value instead of source fallback",
				},
				{
					in: "query",
					name: "excludeStale",
					schema: { type: "string", enum: ["1"] },
					description: "1 = exclude stale translations",
				},
				{
					in: "query",
					name: "limit",
			...
			responses: {
				200: jsonResponse(
					...
					"Bundle entries (and nextCursor when paginated)",
				),
				...errorResponses,
			},
		}),
	...
	projects.post(
		...
		describeRoute({
			...
			responses: {
				200: jsonResponse(
					bulkSetResultSchema,
					"Write summary (count written, keys skipped)",
				),
				...errorResponses,
			},
		}),
	...
	projectOperation("run_qa_checks", {
		...
		responseDescription:
			"QA report (findings grouped by locale + rollup counts)",
	});
				...
				components: {
					securitySchemes: {
						bearerAuth: {
							type: "http",
							scheme: "bearer",
							description: "Your Turjuman API key",
						},
					},
				},
				security: [{ bearerAuth: [] }],
