...
function corpus(): Corpus {
	...
	const concepts = docDocs().filter(
		(d) => d.kind === "concept" && (d.headingPath?.length ?? 1) === 1,
	);
...
export async function searchKnowledge(
	...
	return {
		query: q,
		total,
		hasMore: total > operations.length + docs.length,
		operations,
		docs,
	};
}
...
export function orientation(): SearchResponse {
	const groups: GroupSummary[] = Object.entries(OPERATION_GROUPS).map(
		([group, ops]) => ({
			group,
			count: ops.length,
...
export function describeKnowledge(id: string): DescribeResult {
	if (id.startsWith("op:")) {
		...
		if (!op)
			return {
				error: `Unknown operation: ${id}. Use search to find a valid id.`,
			};
	...
	const text = (byPath.get(chunk.path) ?? [chunk])
		.map((d) =>
			(d.headingPath?.length ?? 1) > 1 ? `## ${d.title}\n\n${d.text}` : d.text,
		)
		.join("\n\n");
	return {
