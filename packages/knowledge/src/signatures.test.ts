...
function probe(field: z.ZodTypeAny, output?: z.ZodTypeAny) {
  return op({ name: "probe", description: "d", input: z.object({ f: field }), output, handler: async () => null });
}

// Oracle 2: hand-authored (zod field) -> (rendered type, optional) rows.
const TYPE_ROWS: { label: string; field: z.ZodTypeAny; type: string; optional: boolean }[] = [
  ...
  { label: "enum", field: z.enum(["m", "n"]), type: '"m" | "n"', optional: false },
  { label: "string array", field: z.array(z.string()), type: "string[]", optional: false },
  { label: "optional string", field: z.string().optional(), type: "string", optional: true },
  { label: "defaulted number", field: z.number().default(5), type: "number", optional: true },
  { label: "nullable-then-optional", field: z.string().nullable().optional(), type: "string", optional: true },
];
...
describe("renderer type table (independent oracle)", () => {
  ...
  it("renders a full multi-field signature in field order", () => {
    const p = op({
      name: "make_thing",
      description: "d",
      input: z.object({ a: z.string(), b: z.number().optional(), c: z.enum(["x", "y"]), d: z.array(z.string()) }),
      handler: async () => null,
    });
    expect(operationSignature(p)).toBe('make_thing(a: string, b?: number, c: "x" | "y", d: string[])');
  });

  it("appends the output shape (top-level keys) and omits it when absent", () => {
    expect(operationSignature(probe(z.string(), z.object({ id: z.string(), n: z.number() })))).toBe(
      "probe(f: string) -> { id, n }",
    );
  ...
  it("renders a no-field input as empty parens", () => {
    expect(operationSignature(op({ name: "ping", description: "d", input: z.object({}), handler: async () => null }))).toBe(
      "ping()",
    );
  });
...
describe("registry signature completeness", () => {
  describe.each(OPERATIONS)("$name", (operation) => {
    it("starts with the op name and lists exactly its input fields", () => {
      expect(operationSignature(operation).startsWith(`${operation.name}(`)).toBe(true);
      const keys = Object.keys((operation.input as z.ZodObject<z.ZodRawShape>).shape ?? {});
