...
describe("expandQuery", () => {
  it("drops stopwords and single-character tokens", () => {
    ...
    for (const dropped of ["how", "do", "i", "a", "new"]) expect(tokens).not.toContain(dropped);
  });
...
describe("searchKnowledge — response shape", () => {
  ...
  it("is deterministic for the same query", async () => {
    ...
    expect(b.operations.map((o) => o.id)).toEqual(a.operations.map((o) => o.id));
  });
});
...
describe("describeKnowledge", () => {
  it("returns full typed detail for an operation id", () => {
    ...
    expect(d).toMatchObject({ kind: "operation", name: "set_translation", readOnly: false, destructive: false });
    if ("input" in d) {
      expect(d.input.find((f) => f.name === "projectId")).toMatchObject({ type: "string", optional: false });
      expect(d.input.find((f) => f.name === "namespace")).toMatchObject({ optional: true });
  ...
  it.each([["op:does_not_exist"], ["guides/nope.mdx#_intro"], ["totally-bogus"]])(
    "returns an error for unknown id %s",
    (id) => {
...
describe("corpus — registry completeness + integrity", () => {
  it("indexes every operation as an `op:<name>` doc", () => {
    ...
    for (const operation of OPERATIONS) {
      expect(ids.has(`op:${operation.name}`), `missing op doc for ${operation.name}`).toBe(true);
    }
  });
  ...
  it("covers multiple Diátaxis doc subtrees", () => {
    ...
    for (const k of ["guide", "concept", "reference"]) expect(kinds.has(k as never)).toBe(true);
  });

  it("every doc chunk carries id, title and source path", () => {
    for (const d of allDocs().filter((x) => x.kind !== "operation")) {
      expect(Boolean(d.id && d.title && d.path), `incomplete chunk ${d.id}`).toBe(true);
    }
  });
});
