import { effectiveAnnotations, type Operation } from "@turjuman/sdk";
...
function renderType(schema: z.ZodTypeAny, depth = 0): string {
	...
	switch (d.typeName) {
		...
		case "ZodEnum":
			return (
				(d.values ?? []).map((v) => JSON.stringify(v)).join(" | ") || "string"
			);
		case "ZodNativeEnum":
			return "enum";
		case "ZodArray": {
		...
		case "ZodUnion":
			return (
				(d.options ?? []).map((o) => renderType(o, depth + 1)).join(" | ") ||
				"unknown"
			);
		case "ZodRecord":
...
function objectShape(schema: z.ZodTypeAny): Record<string, z.ZodTypeAny> {
	...
	const shapeFn = (core as unknown as { shape?: Record<string, z.ZodTypeAny> })
		.shape;
	return shapeFn ?? {};
}
...
export function operationHints(op: Operation): {
	readOnly: boolean;
	destructive: boolean;
} {
