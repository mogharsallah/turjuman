	...
	for (const raw of body.split("\n")) {
		...
		if (!inFence) {
			...
			if (h) {
				...
				const path =
					level === 3 && lastH2 ? [title, lastH2, heading] : [title, heading];
				current = {
					heading,
					anchor: uniqueAnchor(heading),
...
console.log(
	`Built knowledge corpus: ${corpus.length} chunks from ${files.length} docs -> ${relative(repoRoot, outFile)}`,
);
