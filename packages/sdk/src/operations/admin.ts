...
export const adminOps: Operation[] = [
	...
	op({
		...
		handler: (a, { service, actor }) =>
			service.members.list(actor, a.projectId),
	}),
	op({
		name: "add_member",
		description:
		...
		input: z.object({
			...
			name: z
				.string()
				.optional()
				.describe("Display name when provisioning a new user by email"),
			role: projectRole,
		}),
	...
	op({
		...
		input: z.object({
			...
			readOnly: z
				...
				.optional()
				.describe(
					"Limit the key to read-only actions, regardless of the user's role",
				),
			expiresAt: z
				.string()
				.optional()
	...
	op({
		name: "list_api_keys",
		description:
			"List API key metadata (never the secret) for yourself or, as admin, another user.",
	...
	op({
		...
		input: z.object({
			...
			userId: z
				.string()
				.optional()
				.describe("Owner of the key; defaults to yourself"),
		}),
		handler: (a, { service, actor }) =>
			service.apiKeys.revoke(actor, a.apiKeyId, a.userId),
	}),
];
