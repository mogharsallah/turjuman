...
export const scoringOps: Operation[] = [
	op({
		...
		input: z.object({
			...
			comment: z
				.string()
				.optional()
				.describe("One or two sentences citing the main MQM issues."),
			model: z
				.string()
				.optional()
				.describe(
		...
		http: {
			method: "post",
			path: "/v1/projects/:id/translations/score",
			params: { id: "projectId" },
		},
	...
	op({
		...
		http: {
			method: "post",
			path: "/v1/projects/:id/translations/review",
			params: { id: "projectId" },
		},
		handler: (a, { service, actor }) =>
	...
	op({
		...
		handler: async (a, { service, actor }) => {
			const page = await service.scoring.listForReviewPage(
				actor,
				a.projectId,
				a.locale,
				{
	...
	op({
		...
		http: {
			method: "get",
			path: "/v1/projects/:id/score-config",
			params: { id: "projectId" },
		},
		handler: (a, { service, actor }) =>
			service.scoring.getConfig(actor, a.projectId),
	}),
	op({
