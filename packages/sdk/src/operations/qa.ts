...
export const qaOps: Operation[] = [
	op({
		...
		input: z.object({
			...
			locale: localeCode
				.optional()
				.describe("Limit to one locale; omit to check all non-base locales."),
			checks: z
				.array(z.string())
				.optional()
				.describe(
		...
		http: {
			method: "post",
			path: "/v1/projects/:id/checks",
			params: { id: "projectId" },
		},
		handler: (a, { service, actor }) =>
			service.qa.run(actor, a.projectId, {
				locale: a.locale,
				checkIds: a.checks,
				slot: a.slot,
			}),
	...
	op({
		name: "get_qa_config",
		description:
			"Get the project's QA configuration (per-check enable/severity overrides and ignore rules).",
		input: z.object({ projectId }),
		output: qaConfigSchema,
		http: {
			method: "get",
			path: "/v1/projects/:id/qa-config",
			params: { id: "projectId" },
		},
		handler: (a, { service, actor }) =>
			service.qa.getConfig(actor, a.projectId),
	}),
	op({
		name: "set_qa_config",
		description:
		...
		handler: ({ projectId: id, ...input }, { service, actor }) =>
			service.qa.setConfig(actor, id, input),
	}),
];
