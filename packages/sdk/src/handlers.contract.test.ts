...
import { OP_FIXTURES, type OpFixture, makeCtx, spyService } from "./testing/op-fixtures.js";
...
describe("handler arg-mapping contract", () => {
  describe.each(OPERATIONS)("$name", (op) => {
    it("has a fixture", () => {
      expect(OP_FIXTURES[op.name], `no op-fixture for "${op.name}" — add one`).toBeDefined();
    });

    const variants = variantsFor(op.name);
    const labelled = variants.map((v, i) => ({ v, label: v.variant ?? `variant ${i}` }));
    ...
    describe.each(labelled)("$label", ({ v }) => {
      it("fires exactly the expected service call(s)", async () => {
        ...
        expect(calls.length, `${op.name}: wrong number of service calls`).toBe(v.calls.length);
        // Exact methods, order, and every positional arg (incl. the ACTOR slot).
        expect(calls).toEqual(v.calls.map((c) => ({ method: c.method, args: c.args })));
  ...
  it("get_translations throws (no service call) when neither name nor locale is given", async () => {
    ...
    await expect(
      (async () => op.handler({ projectId: "P_projectId" }, makeCtx(service)))(),
...
describe("op-fixture validity (meta)", () => {
  it("no fixture references an unknown operation", () => {
    for (const name of Object.keys(OP_FIXTURES)) {
      expect(OPERATIONS_BY_NAME.has(name), `fixture "${name}" is not a real operation`).toBe(true);
    }
  });
  ...
  describe.each(OPERATIONS)("$name", (op) => {
    ...
    it("each variant validates against op.input AND is already normalized", () => {
      expect(variants.length, "expected at least one variant").toBeGreaterThan(0);
    ...
    it("top-level string fields use distinct sentinels (catches positional swaps)", () => {
      for (const v of variants) {
        const strings = Object.values(v.input).filter((x): x is string => typeof x === "string");
        expect(new Set(strings).size, "duplicate string sentinel in one input").toBe(strings.length);
      }
    });
    ...
    it("variants together populate every input field", () => {
      ...
      for (const field of Object.keys(shape)) {
        expect(covered.has(field), `input field "${field}" is never populated by a fixture`).toBe(true);
      }
    });

    it("every variant expects at least one service call", () => {
      for (const v of variants) {
        expect(v.calls.length, "a variant must assert at least one service call").toBeGreaterThan(0);
      }
    });
  });
});
