...
import { deleteStack, describeStack, findManagedStacks, findStackResource } from "./stack.js";
...
const validationError = () => Object.assign(new Error("does not exist"), { name: "ValidationError" });
        ...
        Stacks: [
          {
            ...
            Outputs: [{ OutputKey: "TableName", OutputValue: "turjuman-Table" }],
...
describe("findManagedStacks", () => {
  it("filters by the managed tag and follows pagination", async () => {
    const pages = [
      {
        Stacks: [
          { StackName: "turjuman", StackStatus: "CREATE_COMPLETE", Tags: [{ Key: "turjuman:managed", Value: "true" }] },
          { StackName: "unrelated", StackStatus: "CREATE_COMPLETE", Tags: [{ Key: "app", Value: "other" }] },
        ],
        NextToken: "page2",
      },
      {
        Stacks: [
          { StackName: "turjuman-staging", StackStatus: "UPDATE_COMPLETE", Tags: [{ Key: "turjuman:managed", Value: "true" }] },
          { StackName: "deleted", StackStatus: "DELETE_COMPLETE", Tags: [{ Key: "turjuman:managed", Value: "true" }] },
        ],
      },
    ];
    let call = 0;
    ...
    expect(found.map((s) => s.stackName)).toEqual(["turjuman", "turjuman-staging"]);
...
describe("deleteStack", () => {
  ...
  it("issues a delete then polls until the stack disappears", async () => {
    ...
    const { send, client } = stubClient({
      [DescribeStacksCommand.name]: () => {
        ...
        describeCalls++;
        if (describeCalls === 1) return { Stacks: [{ StackStatus: "CREATE_COMPLETE" }] };
        if (statuses.length) return { Stacks: [{ StackStatus: statuses.shift()! }] };
        throw validationError();
      },
    ...
    expect(send.mock.calls.some(([cmd]) => cmd instanceof DeleteStackCommand)).toBe(true);
  });
  ...
  it("throws on DELETE_FAILED", async () => {
    ...
    await expect(deleteStack(client, "turjuman")).rejects.toThrow(/DELETE_FAILED/);
  });
  ...
  it("retries with RetainResources when the first delete fails, then succeeds", async () => {
    ...
    const deletes = send.mock.calls.map(([c]) => c).filter((c) => c instanceof DeleteStackCommand);
...
describe("findStackResource", () => {
    ...
    const { client } = stubClient({
      [ListStackResourcesCommand.name]: () => ({
        StackResourceSummaries: [
          { ResourceType: "AWS::Lambda::Function", LogicalResourceId: "ApiFunction" },
          { ResourceType: "AWS::DynamoDB::Table", LogicalResourceId: "Table", PhysicalResourceId: "turjuman-Table-XYZ" },
        ],
      }),
    });
    expect(await findStackResource(client, "turjuman", "AWS::DynamoDB::Table")).toEqual({
      logicalId: "Table",
  ...
  it("returns undefined when no resource matches", async () => {
    ...
    expect(await findStackResource(client, "turjuman", "AWS::DynamoDB::Table")).toBeUndefined();
  });
});
