...
export function migrateConfig(raw: unknown): DeployConfig {
  ...
  if (v1.functionMemorySize != null) functionDefaults.memorySize = v1.functionMemorySize;
    ...
    ...(v1.vpcSubnetIds && v1.vpcSubnetIds.length
      ? {
          vpc: {
            ...
            ...(v1.vpcSecurityGroupIds ? { securityGroupIds: v1.vpcSecurityGroupIds } : {}),
          },
        }
      : {}),
  };
...
export function mapConfigToProps(config: DeployConfig): TurjumanStackProps {
  return {
    ...
    ...(config.functionDefaults ? { functionDefaults: config.functionDefaults } : {}),
...
export function applyOverrides(
  ...
  for (const entry of opts.set ?? []) {
    ...
    if (eq < 0) {
      throw new Error(`Invalid --set "${entry}". Use path=value, e.g. mcp.memorySize=512.`);
    }
...
export function saveLocalConfig(config: DeployConfig, cwd = process.cwd()): string {
  const file = join(cwd, DEPLOY_CONFIG_FILE);
