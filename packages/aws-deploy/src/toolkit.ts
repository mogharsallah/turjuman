 ...
 */
export async function deployStack(opts: DeployStackOptions): Promise<Record<string, string>> {
  if (opts.region) {
  ...
  if (!opts.skipBootstrap) {
    ...
    await toolkit.bootstrap(BootstrapEnvironments.fromCloudAssemblySource(source));
  }
