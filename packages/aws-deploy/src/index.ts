...
async function gatherConfig(existing: DeployConfig | undefined): Promise<DeployConfig> {
  ...
  const stackName = unwrap(
    await p.text({ message: "CloudFormation stack name", initialValue: existing?.stackName ?? "turjuman" }),
  );
  const region = unwrap(
    await p.text({ message: "AWS region", initialValue: existing?.region ?? envRegion ?? "us-east-1" }),
  );
  const cors = unwrap(
    await p.text({
  ...
  const pitr = unwrap(
    await p.confirm({
      message: "Enable point-in-time recovery (continuous backups) on the table?",
  ...
  if (useVpc) {
    ...
    vpc = { subnetIds, ...(securityGroupIds.length ? { securityGroupIds } : {}) };
  }
...
async function resolveBaseConfig(
  ...
  const region =
    opts.region ?? local?.region ?? process.env.AWS_REGION ?? process.env.AWS_DEFAULT_REGION;
  if (stackName && region) {
    ...
    if (ssm) {
      ...
      return {
        config: {
          ...ssm,
          ...(ssm.ownerEmail ?? local?.ownerEmail ? { ownerEmail: ssm.ownerEmail ?? local?.ownerEmail } : {}),
          ...(ssm.ownerName ?? local?.ownerName ? { ownerName: ssm.ownerName ?? local?.ownerName } : {}),
        },
        source: "SSM",
      };
    }
...
async function persistConfig(config: DeployConfig): Promise<void> {
  ...
  try {
    ...
    p.log.warn(`Could not write the canonical config to SSM: ${(err as Error).message}`);
  }
}
...
export async function runDeploy(opts: RunDeployOptions = {}): Promise<void> {
  ...
  if (base && !opts.reconfigure) {
    ...
    p.log.info("Run `turjuman-aws-deploy deploy --reconfigure` to change them.");
    config = base;
  } else {
  ...
  deploySpin.start(opts.skipBootstrap ? "Deploying CloudFormation stack" : "Bootstrapping and deploying");
