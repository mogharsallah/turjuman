...
program
	...
	.description(
		"Turjuman self-host tooling — deploy, inspect, and tear down the AWS stack.",
	)
	.version(version());
...
program
	.command("deploy")
	.description(
		"Deploy (or update) the Turjuman stack to your AWS account and create the first owner.",
	)
	.option(
		"--reconfigure",
		"Re-prompt for settings instead of reusing the saved config",
	)
	...
	.option(
		"--enable <surface>",
		"Enable a surface (api|webhook), then redeploy",
		collect,
		[],
	)
	.option(
		"--disable <surface>",
		"Disable a surface (api|webhook), then redeploy",
		collect,
		[],
	)
	.option(
...
program
	.command("status")
	.description(
		"Show whether Turjuman is installed in the target AWS account/region.",
	)
	.option("--region <region>", "AWS region (defaults to config / AWS_REGION)")
...
program
	.command("teardown")
	.description(
		"Delete the Turjuman stack, its SSM config, and (optionally) local config. Destructive.",
	)
	.option(
		"--stack-name <name>",
		"Stack name (defaults to turjuman.deploy.json)",
	)
	.option("--region <region>", "AWS region (defaults to config / AWS_REGION)")
	.option(
		"--keep-table",
		"Retain the DynamoDB table (and its data) instead of deleting it",
	)
...
program
	.command("bootstrap")
	.description(
		"Create the first owner against a deployed DynamoDB table (for manual deploys).",
	)
	.requiredOption(
		"--table <name>",
		"DynamoDB table name (the TableName stack output)",
	)
	...
	.action(
		async (opts: {
			...
			if (!owner) {
				console.log(
					"Org already has users — bootstrap skipped. Reuse your existing API key.",
				);
				return;
			}
