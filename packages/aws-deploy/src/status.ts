...
export async function runStatus(opts: { region?: string } = {}): Promise<void> {
  ...
  if (existing) {
    ...
    if (info) {
      ...
      const ssm = await loadSsmConfig(region, existing.stackName).catch(() => undefined);
      console.log(`  ${summarizeConfig(ssm ?? existing)}${ssm ? " (from SSM)" : " (from turjuman.deploy.json)"}`);
    } else {
  ...
  if (others.length > 0) {
    ...
    for (const info of others) {
      printStack(info);
      const ssm = await loadSsmConfig(region, info.stackName).catch(() => undefined);
