...
export async function loadSsmConfig(
	...
	try {
		const res = await ssm.send(
			new GetParameterCommand({ Name: ssmConfigPath(stackName) }),
		);
...
export async function saveSsmConfig(
	region: string,
	config: DeployConfig,
): Promise<void> {
	const ssm = new SSMClient(clientConfig(region));
...
export async function deleteSsmConfig(
	region: string,
	stackName: string,
): Promise<void> {
	const ssm = new SSMClient(clientConfig(region));
	try {
		await ssm.send(
			new DeleteParameterCommand({ Name: ssmConfigPath(stackName) }),
		);
	} catch (err) {
