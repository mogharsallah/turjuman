...
import { applyOverrides, type DeployConfig, mapConfigToProps, migrateConfig } from "./config.js";

const base: DeployConfig = { version: 2, stackName: "turjuman", region: "us-east-1" };
...
describe("migrateConfig", () => {
  ...
  it("rejects an invalid config", () => {
    expect(() => migrateConfig({ version: 2, stackName: "", region: "us-east-1" })).toThrow();
  });
});
...
describe("applyOverrides", () => {
  it("enables and disables surfaces", () => {
    expect(applyOverrides(base, { disable: ["api"], enable: ["webhook"] })).toEqual({
      ...base,
      api: { enabled: false },
  ...
  it("rejects an unknown surface", () => {
    expect(() => applyOverrides(base, { enable: ["nope"] })).toThrow(/Unknown surface/);
  });

  it("applies and coerces --set values (number, string, boolean)", () => {
    const next = applyOverrides(base, {
      set: ["mcp.memorySize=512", "table.billingMode=PROVISIONED", "table.pointInTimeRecovery=true"],
    });
    expect(next.mcp).toEqual({ memorySize: 512 });
    expect(next.table).toEqual({ billingMode: "PROVISIONED", pointInTimeRecovery: true });
  });

  it("re-validates through zod, rejecting an invalid value", () => {
    expect(() => applyOverrides(base, { set: ["table.billingMode=NONSENSE"] })).toThrow();
    expect(() => applyOverrides(base, { set: ["bogus"] })).toThrow(/Use path=value/);
  });
});
