...
| **Namespaces & runs**: `NamespaceService` / `RunService` (`core/services/`) + the `namespaces` (`list_namespaces`/`get_namespace`/`create_namespace`/`update_namespace`) / `runs` (`start_run`/`get_run`/`list_runs`/`finish_run`/`cancel_run`) operation groups (`sdk/src/operations/`) — MCP-only (no `http` binding) | `reference/mcp-tools.mdx` (the Namespaces + Runs groups; op names **verbatim**) + `guides/webhooks.mdx` (`run.started`/`run.finished`) + `concepts/how-agents-use-turjuman.mdx` (a run as the recorded agent write job) + `concepts/architecture.mdx` (`Namespace`/`TranslationRun` entities) + `reference/glossary.mdx` (namespace / run vocabulary) |
| **Human-accept principal** (`principal` on `apiKeySchema`/`Actor`; `create_api_key`; the `translations.accept`/`escalations.resolve` gate) | `reference/mcp-tools.mdx` (`create_api_key` `principal`; the agent-can't-accept semantics) + `concepts/roles-and-permissions.mdx` + `self-hosting/security.mdx` (human vs agent keys) + `reference/glossary.mdx` (principal) |
